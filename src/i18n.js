const DICTIONARIES = {
  ja: {
    ...
    'help.step.setup': '1. レート選択とDBU/ノード条件を入力すると Base カードが即時計算されます。',
    'help.step.variant': '2. 比較したい条件は「Variant にコピー」または読込モーダルから Variant に適用してください。',
    'help.step.scenario': '3. 現在の Base は「保存」、Variant は「Variant を保存」でシナリオ登録し、必要に応じて読込/削除します。',
    'help.step.sensitivity': '4. 感度分析をオンにすると ±% を指定して最小・期待値・最大のレンジを確認できます。',
    'help.step.shortcuts': '5. キーボードショートカット: S=保存、V=Variant保存、L=読込、D=削除。',
  ...
  en: {
    ...
    'help.step.setup': '1. Choose rates and usage inputs to calculate the Base card instantly.',
    'help.step.variant': '2. Use "Copy to Variant" or apply a saved scenario to populate the Variant card for side-by-side comparison.',
    'help.step.scenario': '3. Register the current Base with Save, or the Variant with Save Variant, then load or delete from the scenario modal.',
    'help.step.sensitivity': '4. Enable sensitivity to explore ±% swings and review the min / expected / max totals.',
    'help.step.shortcuts': '5. Keyboard shortcuts: S=Save, V=Save Variant, L=Load, D=Delete.',
